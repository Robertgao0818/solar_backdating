...
## 10. 盲评开盲结果与终裁(2026-07-19,owner 裁定以 Codex 首过为准)

盲评执行:owner 通读 30 项(印象:仅 item_16 劣化)+ Codex(gpt-5.6,
经 codex-rescue 通道)独立盲评全部 58 面板,盲态纪律核实(仅访问
scratchpad 合成图,未触碰 answer key/observations)。**Owner 决定
(2026-07-19):Codex 逐项判定采纳为终裁,免除 6 项分歧的人工复看。**

### 10.1 开盲对照(bucket × Codex 判定,n=30)

被门控 16 项(10 oob + 4 conflict + 2 dark_zone):

| 终裁类别 | n | 项目 | 读法 |
|---|---|---|---|
| 正当拦截(真错位,5–16m 偏移经独特地标确证为真) | 6 | 01/02/04/06/18/27(全部 oob) | **净化实证**:absent 在这些帧上不可信,降级正确 |
| 正当拦截(dark_zone,左图自身严重错位) | 2 | 08/15 | 弃权正确 |
| 疑似误杀(conflict 门挡下 ≤1.2m 的真实小修正或无变化帧) | 3 | 03/05/13 | 帧本身基本可用 |
| 估计器出错、帧质量未定(伪峰 11/16/21;不确定 12/26) | 5 | 11/12/16/21/26 | 弃权结果可接受,原因错误 |

对照组 14 项(门控放行):13 项良性(5 真实小改善 + 8 无变化,偏移
≤1.4m);**1 项假放行候选:item_07(放行了 4.3m 校正,Codex 判
degraded)**——放行侧漏检,登记为杠杆 1 的第二靶点(放行侧也过
周期性检查)。

### 10.2 面板级读数(注意:oob 在此 30 项 panel 里被有意过采样,
不可直接外推 corpus 率)

- 净化率(正当拦截)= **8/16 = 50%**(下界;5 项"未定"中若含真错位则更高)
- 误杀率(疑似)= **3/16 ≈ 19%**(全部来自 conflict 门,不是 oob 门)
- 净化 > 误杀,方向满足 G3 门禁的定性形态;oob 门的拦截质量
  (6 正当 / 0 确认误杀 / 4 未定)显著好于 conflict 门(0 正当 /
  3 疑似误杀 / 1 伪峰)。
- 伪峰确认 3/30,与 §5.4 统计诊断(≤12.6% of oob)一致收口。

### 10.3 对 §5 关键警示的最终处置

§5 的自动表("净化 0 / 误杀 95–100%")**被开盲结果推翻**:其"净化"
定义(配准可检出 corrupt/artifact)与实际拦截对象(真实大幅错位)
错配。实证结论:**门控有效,清理对象是"真实错位帧",不是
corrupt/artifact**。
